...
|:---:|:---:|:---:|:---:|
| cpp  | 308 ms | 153.3 MB | 2022/12/25 0:02 |

```cpp

class MagicDictionary {
private:
    unordered_map<string, set<char>> hm;
public:
    MagicDictionary() {
        hm.clear();
    }
    ...
    bool search(string searchWord) {
        ...
        for(int i = 0; i < n; i++){
            ...
            tmp[i] = '*';
            if(this->hm.count(tmp)){
                if(!this->hm[tmp].count(searchWord[i]) || this->hm[tmp].size() > 1) return true;
            }
        }
        return false;
    }
