...
|:---:|:---:|:---:|:---:|
| cpp  | 1264 ms | 8.6 MB | 2022/10/26 23:19 |

```cpp

class Solution {
public:
    int dfs(vector<int>& nums, int t, int c, int r){
        if(c == nums.size()) return t == r;

        int p = dfs(nums, t, c + 1, r + nums[c]);
        int n = dfs(nums, t, c + 1, r - nums[c]);
        return p + n;
    }
