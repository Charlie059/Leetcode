...
|:---:|:---:|:---:|:---:|
| cpp  | 4 ms | 8 MB | 2023/01/15 16:11 |
...
public:
    ...
    double dfs(string nodeA, string nodeB, unordered_map<string, unordered_map<string, double>>& g, unordered_set<string>& visited){
        if(nodeA == nodeB) return 1.0;
        visited.insert(nodeA);
        for(auto n: g[nodeA]){
            if (visited.count(n.first)) continue;
            double ans = 1;
            ans = ans * dfs(n.first, nodeB, g, visited);
            if(ans >= 0) return ans * n.second;
        }
    ...
    vector<double> calcEquation(vector<vector<string>>& equations, vector<double>& values, vector<vector<string>>& queries) {
        ...
        for(auto q: queries){
            ...
            if(!g.count(nodeA) || !g.count(nodeB)) ans.emplace_back(-1);
            else ans.emplace_back(dfs(nodeA, nodeB, g, visited));
        }
