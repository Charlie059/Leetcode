...
[LeetCode](https://leetcode.com/problems/number-of-provinces/description/)  -  [LeetCode-CN](https://leetcode.cn/problems/number-of-provinces/description/)
## Latest Accepted Submissions - 最近一次 AC 的提交


| Language | Runtime | Memory | Submission Time |
|:---:|:---:|:---:|:---:|
| cpp  | 8 ms | 13.4 MB | 2022/02/12 8:13 |

```cpp

class Solution {
public:
    void dfs(vector<vector<int>>& isConnected,  vector<int> & vistied, int i){
        int n = isConnected.size();
        for(int j = 0; j < n; j++){
            if(vistied[j] == 0 && isConnected[i][j] == 1){
                vistied[j] = 1;
                dfs(isConnected, vistied, j);
            }
        }
    }
    int findCircleNum(vector<vector<int>>& isConnected) {
        int rows = isConnected.size();
        vector<int> vistied(rows, 0);
        int ans = 0;
        for(int i = 0; i < rows; i++){
            if(vistied[i] == 0){
                dfs(isConnected, vistied, i);
                ans++;
            }
        }
        return ans;
