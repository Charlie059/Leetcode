...
|:---:|:---:|:---:|:---:|
| cpp  | 4 ms | 6.4 MB | 2023/01/23 16:31 |
...
public:
    ...
    void dfs(const string& digits, int idx, const int n, unordered_map<int, string>& hm){
        if(path.length() == n){
            ans.emplace_back(path);
            return;
        }

        for(int i = 0; i < hm[digits[idx] - '0'].size(); i++){
            path += hm[digits[idx] - '0'][i];
            dfs(digits, idx + 1, n, hm);
            path.pop_back();
        }
    }
