...
|:---:|:---:|:---:|:---:|
| cpp  | 44 ms | 33.6 MB | 2022/12/12 21:36 |
...
public:
    int leastInterval(vector<char>& tasks, int n) {
        ...
        for(int i = 0; i < m; i++)  taskCnt[tasks[i] - 'A']++;
        

        int k = *max_element(taskCnt.begin(), taskCnt.end());
        int ans = (k - 1) * (n + 1);
        ans += count_if(taskCnt.begin(), taskCnt.end(), [&](int i){return i == k;});
